(function() {
  ...
  _.each = function(collection, iterator) {
	if(Array.isArray(collection)){
		...
		iterator(collection[iter], iter, collection);
		}
	}
	else{
  ...
  _.uniq = function(array) {
	var result = [];
	_.each(array, function(val, iter, array){
		var uniq=1;
  ...
  _.reduce = function(collection, iterator, accumulator) {
	...
	_.each(collection, function(arg){
		accumulator = iterator(accumulator,arg);
		});
	return accumulator;
  };
  ...
  _.every = function(collection, iterator) {
	...
	return _.reduce(collection, function(collector, arg){
		if(iterator===undefined){
			if(!arg){	
			collector = false;}
		}
		else if(!iterator(arg)){
			collector = false;
  ...
  _.extend = function(obj) {
		_.each(arguments,function(value,iter){
  ...
  _.shuffle = function(array) {
	...
	for(var i=0; i<copy.length; i++){
		var temp = undefined;
		var rand = Math.ceil(Math.random()*copy.length-1);
